---
name: researcher
description: "Parallel internet research agent for research-report workflows. Searches credible sources, reads web content, and returns structured findings summaries. Use for researching topics, verifying claims, and gathering evidence from the internet."
...
**Note**: When dispatched via the research command (`/research-report:research`), this strategy is handled by the dedicated `methodological-critic` agent (Opus), not this researcher agent. The section below exists as a fallback specification and for consistency documentation.
