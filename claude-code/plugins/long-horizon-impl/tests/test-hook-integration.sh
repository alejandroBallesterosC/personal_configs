...
echo "=========================================="
echo "Integration tests: long-horizon-impl hooks"
echo "=========================================="
echo ""
...
# --- Test 9: Stop hook with active research-plan workflow logs action ---
echo ""
...
CONTENT=$(cat "$LOG")
cd - > /dev/null
assert_contains "$CONTENT" "test-project\|iteration\|block" "log records action for active research-plan workflow"
teardown

# --- Test 10: Stop hook with implement workflow logs ralph-loop pass-through ---
echo ""
...
CONTENT=$(cat "$LOG")
cd - > /dev/null
assert_contains "$CONTENT" "ralph-loop\|lhi-implement\|Allowing stop" "log records ralph-loop pass-through"
teardown

# --- Test 11: Stop hook with waiting_for_input logs B0 pause ---
echo ""
...
iteration: 10
total_iterations_research: 30
total_iterations_planning: 1
research_budget: 30
planning_budget: 20
...
CONTENT=$(cat "$LOG")
cd - > /dev/null
assert_contains "$CONTENT" "waiting_for_input\|Allowing stop" "log records waiting_for_input pause"
teardown

# --- Test 12: Log files do not contain autonomous-workflow references ---
echo ""
...
# --- Test 13: Both hooks use correct log file names ---
echo ""
echo "--- Test 13: Both hooks use correct log file names ---"
setup
cd "$TEST_DIR"
echo '{}' | bash "$STOP_HOOK" 2>/dev/null
echo '{"source":"compact"}' | bash "$AUTO_RESUME" 2>/dev/null
