...
create_research_state() {
  ...
  local command="${7:-/long-horizon-impl:research-plan '$topic' 'test prompt' --research-iterations $research_budget}"
...
create_implementation_state() {
  ...
  local command="${9:-/long-horizon-impl:research-plan '$topic' 'test prompt' --research-iterations $research_budget --plan-iterations $planning_budget}"
  mkdir -p "$TEST_DIR/.claude"
...
reset_test_dir
create_research_state "my-topic" "in_progress" "lhi-research-plan" 5 50 10 "/long-horizon-impl:research-plan 'my-topic' 'test prompt' --research-iterations 50"
EXIT_CODE=0
run_hook || EXIT_CODE=$?
...
assert_output_contains "$OUTPUT" "research-plan" "reason contains the command"
...
reset_test_dir
create_implementation_state "impl-topic" "in_progress" "lhi-implement" 0 0 0 0 25 "/long-horizon-impl:implement 'impl-topic'"
EXIT_CODE=0
run_hook || EXIT_CODE=$?
...
research_budget: 50
command: |
  /long-horizon-impl:research-plan 'corrupt' 'test'
---
...
reset_test_dir
create_implementation_state "impl-done" "complete" "lhi-implement" 0 0 0 0 25 "/long-horizon-impl:implement 'impl-done'"
...
reset_test_dir
create_implementation_state "impl-pending" "complete" "lhi-implement" 0 0 0 0 25 "/long-horizon-impl:implement 'impl-pending'"
...
create_implementation_state "priority-test" "in_progress" "lhi-research-plan" 50 50 20 20 25 "/long-horizon-impl:research-plan 'priority-test' 'test' --research-iterations 50 --plan-iterations 20"
EXIT_CODE=0
run_hook || EXIT_CODE=$?
assert_valid_json "$OUTPUT" "output is valid JSON"
assert_json_field "$OUTPUT" '.decision' "block" "decision is block"
# The command should reference the implementation state, not research
assert_output_contains "$OUTPUT" "research-plan" "blocks with implementation state command"
echo ""
...
create_research_state "edge-case" "in_progress" "lhi-research-plan" 5 50 10 "/long-horizon-impl:research-plan 'edge-case' 'test prompt' --research-iterations 50"
...
assert_output_contains "$OUTPUT" "research-plan" "blocks with research command (not cleanup)"
