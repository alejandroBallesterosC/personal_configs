#!/bin/bash
# ABOUTME: Stop hook for long-horizon-impl plugin that drives iteration (re-feeds command on in_progress) and verifies completion criteria per workflow type.
...
# Find active long-horizon-impl workflow state files
...
if [ -z "$ACTIVE_STATE" ]; then
  debug_log "**Allowing stop:** No active long-horizon-impl workflow found"
  exit 0
fi
...
debug_log "**Fields:** status=$STATUS, iteration=$ITERATION, workflow_type=$WORKFLOW_TYPE, name=$NAME, current_phase=$CURRENT_PHASE, command=$(echo "$COMMAND" | head -1)"

# ---------------------------------------------------------------
# STATUS: waiting_for_input — allow stop (human needs to answer scoping interview questions)
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
if [ "$STATUS" = "in_progress" ]; then
  # lhi-implement uses ralph-loop for iteration, not the stop hook.
  # Allow stop so ralph-loop can drive the next iteration.
  if [ "$WORKFLOW_TYPE" = "lhi-implement" ]; then
    debug_log "**Allowing stop:** lhi-implement uses ralph-loop for iteration, not stop hook."
    exit 0
  fi
  ...
  # Phase A → Phase B transition (safety net for lhi-research-plan workflows)
  # If research budget is reached but phase is still A, transition to Phase B
  if [ "$WORKFLOW_TYPE" = "lhi-research-plan" ] && \
    ...
    yq --front-matter=process '.current_phase = "Phase B: Planning"' "$ACTIVE_STATE" > "$TEMP_PHASE"
    mv "$TEMP_PHASE" "$ACTIVE_STATE"
    debug_log "**Phase transition (safety net):** Phase A → Phase B for $NAME"
  fi
  ...
  SYSTEM_MSG="Continuing long-horizon-impl workflow iteration $NEXT_ITERATION. State file: $ACTIVE_STATE"
...
if [ "$STATUS" = "complete" ]; then
  ...
  # Extract topic name from state file for feature-list path lookup
  TOPIC_NAME="$NAME"

  case "$WORKFLOW_TYPE" in
    lhi-research-plan)
      # Verify: research budget fulfilled
      ...
      # Verify: planning budget fulfilled
    ...
    lhi-implement)
      ...
      if [ -f "$FEATURE_LIST" ]; then
        ...
        # Warn if features are blocked but status is complete
        if [ "$BLOCKED_FEATURES" -gt 0 ] 2>/dev/null; then
          debug_log "**WARNING:** $BLOCKED_FEATURES features are blocked but status is complete. This indicates a command logic error."
        fi
      fi
      ;;
