...
This skill provides **navigation guidance** for the TDD implementation workflow plugin's 8 phases (Phases 2-9). Based on practices from Boris Cherny (Claude Code creator), Thariq Shihab (Anthropic), Mo Bitar, and Geoffrey Huntley.

**Important:** This skill is the **source of truth** for understanding the workflow (overview, principles, context management, state file format). The command files (`1-start-tdd-implementation.md`, `7-implement.md`, etc.) contain the **execution instructions** only.
...
Activate when:
- User asks about the TDD implementation workflow process
- User seems lost in the workflow
- Navigating between workflow phases
- User needs help understanding workflow phases

**Note:** When executing the workflow via `/dev-workflow:1-start-tdd-implementation`, follow the instructions in that command directly - this skill is supplementary guidance.

**Announce at start:** "I'm using the tdd-implementation-workflow-guide skill to help navigate this workflow."
...
```bash
/dev-workflow:continue-workflow <feature-name>
```
...
**Note:** These summaries explain what each phase does. For **execution instructions**, see the individual phase commands (`2-explore.md`, `3-user-specification-interview.md`, etc.). The `1-start-tdd-implementation.md` command orchestrates all phases in sequence.
...
**Command**: `/dev-workflow:7-implement <feature> "<description>"`
...
**Command**: `/dev-workflow:8-e2e-test <feature> "<description>"`
...
| `/dev-workflow:2-explore <name> "<desc>"` | Parallel exploration (5 agents) |
...
| `/dev-workflow:6-review-plan <name>` | Challenge plan |
| `/dev-workflow:7-implement <name> "<desc>"` | TDD implementation |
| `/dev-workflow:8-e2e-test <name> "<desc>"` | E2E testing |
| `/dev-workflow:9-review <name>` | Parallel review (5 agents) |
| `/dev-workflow:continue-workflow <name>` | **Continue an in-progress workflow** |
...
When user invokes `/dev-workflow:1-start-tdd-implementation <feature> "<description>"`:
1. The `1-start-tdd-implementation.md` command orchestrates all phases in sequence
