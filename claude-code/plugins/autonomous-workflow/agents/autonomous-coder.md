...
# ABOUTME: Spawned by Mode 3 (implement) to implement one feature at a time. BLOCKS on external issues instead of mocking around them.
...
Write a structured escalation entry to the escalation file path provided in your task (or `.claude/autonomous-<project>-escalations.json` by default):
