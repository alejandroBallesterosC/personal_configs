// ABOUTME: Reusable utility functions for Playwright browser automation.
// ABOUTME: Provides helpers for clicks, typing, screenshots, server detection, and more.
...
async function launchBrowser(browserType = 'chromium', options = {}) {
  // Default to visible browser (headless: false) unless HEADLESS=true is set
 ...
 * @param {Object} options - Screenshot options (path overrides default /tmp location)
 */
...
async function scrollPage(page, direction = 'down', distance = 500) {
  ...
  // Brief pause for scroll animation - intentional fixed delay for visual feedback
