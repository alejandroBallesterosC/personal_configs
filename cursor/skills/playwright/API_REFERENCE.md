...
# Install (if needed)
cd ~/.cursor/skills/playwright
npm run setup
```

### Basic Configuration (Optional)

For projects using `@playwright/test`, create `playwright.config.js`:
...
async function scrollToBottom(page) {
  ...
  // Brief pause for content to load - use explicit setTimeout for intentional delays
