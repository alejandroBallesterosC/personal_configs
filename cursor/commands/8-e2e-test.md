...
Use Task tool with subagent_type='test-designer':
```
Feature: $1
...
Use the playwright skill to understand how to use Playwright to test the application end-to-end if the application has a frontend/UI that needs to be tested, otherwise stick to other testing frameworks more suitable for end-to-end backend-only testing.
...
```
/ralph-loop "Fix E2E test failures for $1 using orchestrated approach.
