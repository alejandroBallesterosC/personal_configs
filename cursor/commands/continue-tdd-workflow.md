...
To start a workflow, use:
  /1-start <feature-name> "<feature description>"
```
...
2. Run: /1-start $1 "<feature description>"
```
...
**REQUIRED**: Use the Skill tool to invoke `tdd-workflow-guide` to load workflow context.
...
```
/ralph-loop "Continue implementing [Next Component] for $1 using orchestrated TDD.
...
This command (`/continue-tdd-workflow`) is for **manual continuation** in scenarios where:
...
Phases 7, 8, and 9 require the `/ralph-loop` command. This is built-in via the commands and hooks synced to `~/.cursor/`.
