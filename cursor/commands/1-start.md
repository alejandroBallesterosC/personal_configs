...
1. **Load workflow context**: Use the Skill tool to invoke `tdd-workflow-guide`
...
2. **Verify ralph-loop command** is available (required for Phases 7, 8, 9)
   - Check that the `/ralph-loop` command is available
   - If not available, inform the user to sync cursor components to `~/.cursor/`
