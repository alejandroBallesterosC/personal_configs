...
1. **Check architecture exists** at `docs/workflow-$1/plans/$1-architecture-plan.md`
   - If not, recommend running `/4-plan-architecture $1` first

2. **Check specification exists** at `docs/workflow-$1/specs/$1-specs.md`
   - If not, recommend running `/3-user-specification-interview $1 "<description>"` first
...
Next step:
/6-review-plan $1 (challenge the plan before implementation)
```
