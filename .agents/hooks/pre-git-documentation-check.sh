...
# shellcheck source=.agents/hooks/hook-common.sh
. "$SCRIPT_DIR/hook-common.sh"

# Required documentation files, as "path|description" entries relative to the repo root.
# These files must always exist and be non-empty before any commit/PR mutation is allowed.
# Edit this list to change which files are enforced. An empty list ( REQUIRED_DOC_FILES=() )
# disables file-specific enforcement; the root and subdirectory AGENTS.md/CLAUDE.md pairing
# checks always run regardless.
REQUIRED_DOC_FILES=(
  # "docs/required-docs-file.md|description"
)
